...
use crate::{
    callback::{PyCallbackOutput, self}, ffi, impl_::panic::PanicTrap, methods::IPowModulo,
    panic::PanicException, types::PyModule, GILPool, IntoPyPointer, Py, PyResult, Python, catch_unwind_filtered,
};
    ...
    R: PyCallbackOutput,
{
    ...
    let out = panic_result_into_callback_output(
        py,
        catch_unwind_filtered(move || -> PyResult<_> { body(py) }),
    );
    trap.disarm();
    out
}
